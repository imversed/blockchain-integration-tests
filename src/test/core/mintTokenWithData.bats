...
hash=$(yes | imversed tx nft issue nikotestingtokens --from="$address" --name=nikotestingtokens --mint-restricted=false --update-restricted=false --chain-id=imversed-test-1 --fees=200nimv --oracle-url="$oracle_url" --schema==https://metachain-web-staging.fdvr.co/nft/schemas/schema.json --node=http://metachain-staging.fdvr.co:26657 | grep -o 'txhash:.*')
