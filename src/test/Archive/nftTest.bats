...
sleep 6
mrc=$(yes | imversed tx nft mint "$denom"  "$token"  --recipient="$address" --from="$address" --node=http://metachain-staging.fdvr.co:26657 --fees=200nimv | grep -o 'txhash:.*')
...
sleep 5
trc=$(yes | imversed tx nft transfer "$addressTransferTest" "$denom" "$token" --from="$address" --node=http://metachain-staging.fdvr.co:26657 --fees=200nimv | grep -o 'txhash:.*')
...
echo "$txhash" >&3
sleep 6
responseCode=$(imversed q tx --type=hash "$txhash" --node=http://metachain-staging.fdvr.co:26657 | grep -o 'code:.*')
echo "$responseCode" >&3
[ "$responseCode" == "code: 0" ]
}
